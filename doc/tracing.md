# User-space, Statically Defined Tracing (USDT) for FixedCoin Core

FixedCoin Core includes statically defined tracepoints to allow for more
...
**Note**: FixedCoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to FixedCoin Core
...
Multiple tools can list the available tracepoints in a `fixedcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in FixedCoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/fixedcoind
stap net        outbound_message 0x0000000000107c05 /src/fixedcoind
stap validation block_connected  0x00000000002fb10c /src/fixedcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in FixedCoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/fixedcoind | grep NT_STAPSDT -A 4 -B 2
