FixedCoin Core version 0.11.0 is now available from:

  <https://fixedcoin.org/bin/fixedcoin-core-0.11.0/>
...
For example, add the following to `fixedcoin.conf`:
...
related to the blockchain in the fixedcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows FixedCoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that FixedCoin Core will 
...
For an example script see [fixedcoin-submittx](https://github.com/laanwj/fixedcoin-submittx).
...
- #5334 `cf87536` libfixedcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new fixedcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only FixedCoinAmountField
- #5753 `0f44672` Add fixedcoin logo to about screen
...
- #5809 `46bfbe7` Add fixedcoin-cli man page
...
- #5849 `41259ca` contrib/init/fixedcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename FixedCoin-Qt.app to FixedCoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for fixedcoin-tx)
