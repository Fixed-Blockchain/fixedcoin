# CJDNS support in FixedCoin Core

It is possible to run FixedCoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of FixedCoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the FixedCoin network and individual nodes.
...
before it will work with your FixedCoin Core node. This is described in steps
...
## Run FixedCoin Core with CJDNS

Once you are connected to the CJDNS network, the following FixedCoin Core
...
When enabled, this option tells FixedCoin Core that it is running in an
...
IPv6 local network. This helps FixedCoin Core perform better address management:
...
CJDNS support was added to FixedCoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `fixedcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/fixedcoin/fixedcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in FixedCoin Core

There are several ways to see your CJDNS address in FixedCoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `fixedcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `fixedcoin-cli getpeerinfo`).
