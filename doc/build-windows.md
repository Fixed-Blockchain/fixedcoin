...
Below are some notes on how to build FixedCoin Core for Windows.

The options known to work for building FixedCoin Core on Windows are:
...
Note that for WSL the FixedCoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/fixedcoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\fixedcoin`, for example:

    make install DESTDIR=/mnt/c/workspace/fixedcoin
