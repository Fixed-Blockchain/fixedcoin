...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\FixedCoin
    //   new: C:\Users\Username\AppData\Local\FixedCoin
    // macOS: ~/Library/Application Support/FixedCoin
    // Unix-like: ~/.fixedcoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "FixedCoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "FixedCoin";
#else
    fs::path pathRet;
