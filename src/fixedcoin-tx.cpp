...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " fixedcoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The fixedcoin-tx tool is used for creating and modifying fixedcoin transactions.\n\n"
                "fixedcoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded fixedcoin transaction, or with \"-create [commands]\" to create a hex-encoded fixedcoin transaction.\n"
                "\n"
                "Usage: fixedcoin-tx [options] <hex-tx> [commands]\n"
                "or:    fixedcoin-tx [options] -create [commands]\n"
                "\n";
